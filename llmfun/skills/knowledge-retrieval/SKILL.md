...
`listRAGDatabases`, `listRAGSources`, `queryTextSearch`, `querySemantic`,
`queryBestMatch`, `queryReadFile`, and `readRAGSource`.
...
3. **Phase 2: Dig** (Calls 5-8) — Read specific lines from relevant documents; read a referenced document in full with `readRAGSource`.
...
| `listRAGSources` | Resolve the exact indexed path of a document you know by name; when results only *mention* the document you need | When you already know the exact stored path |
| `readRAGSource` | Read a whole document, e.g. one that another document referenced | For tiny lookups — prefer `queryReadFile` |
| `queryReadFile` | Read a specific line from a known file in the RAG | When you don't know the file or line number |
...
- **Referenced documents**: When a result mentions another document by name (e.g. "see auth_token_spec.md"), or the user asks for "the file X": call `listRAGSources` with a `filter` to resolve the exact indexed path, then `readRAGSource`. Bare file names also work directly with `readRAGSource` and `queryReadFile`.
